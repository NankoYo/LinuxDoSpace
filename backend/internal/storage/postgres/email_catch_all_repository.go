...
// inside the same write statement. This closes the classic read-then-update
// race where concurrent deliveries could otherwise overshoot the cap.
func upsertEmailCatchAllDailyUsageTx(ctx context.Context, tx *queryTx, userID int64, usageDate string, count int64, effectiveDailyLimit int64, now time.Time) (model.EmailCatchAllDailyUsage, error) {
...
func upsertEmailCatchAllAccessTx(ctx context.Context, tx *queryTx, userID int64, subscriptionExpiresAt *time.Time, remainingCount int64, temporaryRewardCount int64, temporaryRewardExpiresAt *time.Time, dailyLimitOverride *int64, now time.Time) error {
