...
`,
	...
	return s.getEmailTargetByID(ctx, item.ID)
}
...
func countEmailTargetVerificationAttemptsTx(ctx context.Context, tx *sql.Tx, ownerUserID int64, email string, shortWindowStart time.Time, dailyWindowStart time.Time) (int, int, int, int, error) {
	row := tx.QueryRowContext(ctx, `
SELECT
